...
<script>
import { listQuestion, getQuestion, delQuestion, addQuestion, updateQuestion } from "@/api/app/english/question";

import AppBase from "../../app";
export default {
  mixins: [AppBase],
  name: "Question",
  dicts: ['app_lesson_unit', 'app_primary_grade', 'app_english_question_type', 'app_term'],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      app_term: [],
      app_lesson_unit: [],
      app_english_question_type: []
    };
  },
  created() {
    this.getList();
    this.getDictData(["app_primary_grade", "app_term", "app_lesson_unit", "app_english_question_type"]);
  },
  methods: {
    /** 查询英语错题主列表 */
    getList() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('english/question/export', {
        ...this.queryParams
